...
class DailyMealPlanner extends StatelessWidget {
  ...
  DailyMealPlanner(this.mealsCubit,
      {@required this.date, @required this.meals, this.mealsContext}) {
    meals = meals
        .where(
          (meal) => mealOnCurrentDay(meal, date),
        )
        .toList();
    meals.forEach((meal) {
      if (meal.accepted) {
    ...
    _proposedMeals.sort(
        (a, b) => getDifferenceOfVotes(b).compareTo(getDifferenceOfVotes(a)));
  }
                    ...
                    delegate: SliverChildBuilderDelegate((context, index) {
                      return Column(
                        children: [
                          PlannedMealsList(
                            mealsCubit,
                            meals: _acceptedMeals,
                            isLeaderboard: false,
                            mealContext: mealsContext,
                          ),
                          PlannedMealsList(
                            mealsCubit,
                            meals: _proposedMeals,
                            isLeaderboard: true,
                            mealContext: mealsContext,
                            addToAccepted: addMealToAcceptedList,
                            deleteFromUnaccepted: deleteMealFromUnaccepted,
                          )
                        ],
                      );
                    }, childCount: 1),
