...
class UpdateRecipeCubit extends Cubit<UpdateRecipeState> {
  ...
  UpdateRecipeCubit(this._recipeRepository, this._ingredientRepository)
      : super(UpdateRecipeInitial());
  ...
  Future emitInteraction() async {
    emit(UpdateRecipeInteraction(
        _name, _description, _cookingTimeInMin, _allIngredients,
        selectedImage: _selectedImageFile,
        selectedIngredients: _selectedIngredients,
        oldImageUri: _oldImageUri));
  }

  Future<Recipe> submit(String name, int cookingTimeInMin, String description,
      String imageUrl) async {
    emit(UpdateRecipeSubmitting());
    try {
      var recipeResult = await _recipeRepository.put(
          Recipe(
              id: _recipeId,
              name: name,
