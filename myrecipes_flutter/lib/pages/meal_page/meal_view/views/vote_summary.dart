...
class VoteSummaryState extends State<VoteSummary>{
  ...
  void initState() {
    ...
    positiveVotes = widget.meal.votes
        .where((e) => e.voteIsPositive)
        .length;
    negativeVotes = widget.meal.votes
        .where((e) => e.voteIsPositive)
        .length;
          ...
          Row(
            children: [
              GestureDetector(
                  ...
                  child: Icon(Icons.arrow_drop_up_outlined, color: userVote??true ? Colors.green : Colors.grey,
                      size: 24)),
              Text("$positiveVotes"),
            ],
          ),
          Row(
            children: [
              GestureDetector(
                  ...
                  child: Icon(Icons.arrow_drop_down_outlined, color: !(userVote??false) ? Colors.red : Colors.grey,
                    size: 24,)),
              Text("$negativeVotes"),
            ],
          )
  ...
  bool _getUserVote(BuildContext context) {
    String email = RepositoryProvider.of<AuthRepository>(context).authState.email;
    var first = widget.meal.votes.firstWhere((element) => element.username.toUpperCase() == email.toUpperCase(), orElse: () => null,);
    return first?.voteIsPositive;
  }

  _vote(BuildContext context, bool isPositive) async {
    await RepositoryProvider.of<MealRepository>(context).vote(widget.meal.mealId, isPositive);
  }
}